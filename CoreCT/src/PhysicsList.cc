...
void PhysicsList::ConstructOp()
{
  ...
  while( (*theParticleIterator)() ){
    ...
    if (particleName == "opticalphoton") {
      pmanager->AddDiscreteProcess(theAbsorptionProcess);         // поглощение средой
      pmanager->AddDiscreteProcess(theRayleighScatteringProcess); // рассеяние средой
      pmanager->AddDiscreteProcess(theBoundaryProcess);           // преломление и отражение
    }
  }
}
